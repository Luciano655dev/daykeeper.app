...
export default function ReportPostModal({ postId, open, onClose }: Props) {
  ...
  async function submit() {
    ...
    try {
      const res = await apiFetch(`${API_URL}/post/${postId}/report`, {
        method: "POST",
        cache: "no-store",
      ...
      if (!res.ok) {
        // make the thrown error.message JSON so your parsing pattern works
        const payloadText = await res.text().catch(() => "")
        throw new Error(
          payloadText ||
            JSON.stringify({ message: `Request failed (${res.status})` })
        )
      }
            ...
            >
              <div className="flex items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-(--dk-ink) font-semibold">Report post</h2>
                </div>
              ...
              <p className="text-sm text-(--dk-slate) mb-3">
                Tell us why you’re reporting this (optional).
              </p>
